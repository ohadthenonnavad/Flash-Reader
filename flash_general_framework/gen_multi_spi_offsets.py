...
# Import the single-platform helpers from the local shim, which loads the legacy tools parser
        ...
        for sku in root.findall('.//sku'):
            ...
            if did:
                try:
                    ...
                    pass
        return sorted(list({d for d in dids}))
    except Exception:
        return []
...
def emit_multi_header(out_path: str, items: List[Tuple[str, dict, dict, dict, List[int]]]):
    ...
    for idx, (sku_code, bar, regs, consts, dids) in enumerate(items):
        ...
        for k in ['bus','dev','fun','reg','width','mask','size','fixed_address','offset']:
            v = bar.get(k, 0)
            if k in ('bus','dev','fun','reg','width'):
                lines.append(f'    .{k} = {int(v)},')
            else:
                lines.append(f'    .{k} = 0x{int(v):X}ULL,')
        lines.append('};')
        ...
        lines.append('};')
        arr = ', '.join(f'0x{d:04X}' for d in (dids or []))
    ...
    lines.append('static const struct spi_platform __ro_after_init g_spi_platforms[] = {')
    for idx, _ in enumerate(items):
        lines.append('    { .bar = &g_spibar_meta_%d, .regs = &g_spi_regs_%d, .c = &g_spi_consts_%d, .sku = g_spi_sku_%d, .dids = g_spi_dids_%d, .did_count = (u32)(sizeof(g_spi_dids_%d)/sizeof(g_spi_dids_%d[0])) } ,' % (idx, idx, idx, idx, idx, idx, idx))
    lines.append('};')
    lines.append('static const u32 __ro_after_init g_spi_platforms_count = (u32)(sizeof(g_spi_platforms)/sizeof(g_spi_platforms[0]));')
...
def main():
    ...
    for pch in pchs:
        ...
        # Defaults; optional CHIPSEC constants if available
